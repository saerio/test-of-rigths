...
class TestAuths(AbstractPostgresTest):
    ...
    def test_get_session_user(self):
        ...
        assert response.json() == {
            "id": "1",
            "name": "John Doe",
            "email": "john.doe@saensamecraftui.com",
            "role": "user",
    ...
    def test_update_password(self):
        ...
        old_auth = self.auths.authenticate_user(
            "john.doe@saensamecraftui.com", "old_password"
        )
        assert old_auth is None
        new_auth = self.auths.authenticate_user(
            "john.doe@saensamecraftui.com", "new_password"
        )
    ...
    def test_signin(self):
        ...
        response = self.fast_api_client.post(
            ...
            json={"email": "john.doe@saensamecraftui.com", "password": "password"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == user.id
        assert data["name"] == "John Doe"
        assert data["email"] == "john.doe@saensamecraftui.com"
    ...
    def test_add_user(self):
        with mock_webui_user():
            response = self.fast_api_client.post(
                ...
                json={
                    "name": "John Doe 2",
                    "email": "john.doe2@saensamecraftui.com",
                    "password": "password2",
                    "role": "admin",
                },
            )
        ...
        assert data["email"] == "john.doe2@saensamecraftui.com"
